...
void EmuRec_SetPageProtection(void* address, size_t size)
{
	...
	do {
		...
		retval = VirtualProtect(address, markSize, PAGE_EXECUTE_READWRITE, &oldProtectFlags);
...
void EmuRec_Init(EMU_U32 Size)
{
	...
	EmuRec_SetPageProtection(RecMemory.Memory, RecMemory.Size);
	EmuRec_Reset();
}
