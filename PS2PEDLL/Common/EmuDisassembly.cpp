...
DLLEXPORT BOOL CALLBACK EmuFindLabel( char * Label, EMU_U32 * LabelIndex )
{
    ...
    if ( ElfFile->Header.e_shnum > 0 )
    {
        for ( int i = 0; i < ElfFile->Header.e_shnum; i++ )
        {
            if ( ! strncmp( &ElfFile->SessionSTable[ ElfFile->Session[ i ].sh_name ],
                            Label, strlen( Label ) ) )
            {
...
DLLEXPORT void CALLBACK EmuDisassembly( EMU_U32 Address, char * Buffer, EMU_U32 BufferSize )
{
    ...
    {
        strcat( stDisassembly, (*EmuInstructions[ Index ].Disassembly)( Address, tInstruction ) );
    }
...
char * EmuDis_0_RT_I16( EMU_U32, EMU_U32 Code )
{
    ...
    sprintf( Local_dis_str, " %s, $%x", EmuDis_GetRegName( RT ), I16 );
...
char * EmuDis_PREF( EMU_U32, EMU_U32 Code )
{
    ...
    switch ( HINT )
    {
    case 0:     strcpy( Hint_str, "load" );                 break;
    case 1:     strcpy( Hint_str, "store" );                break;
    case 4:     strcpy( Hint_str, "load_streamed" );        break;
    case 5:     strcpy( Hint_str, "store_streamed" );       break;
    case 6:     strcpy( Hint_str, "load_retained" );        break;
    case 7:     strcpy( Hint_str, "store_retained" );       break;
    case 25:    strcpy( Hint_str, "writeback_invalidate" ); break;
    default:    strcpy( Hint_str, "nop" );                  break;
    }
...
char * EmuDis_FS_FD( EMU_U32, EMU_U32 Code )
{
    ...
    sprintf( Local_dis_str, " FPR[ %d ], FPR[ %d ]",
            FD,
            FS );
...
char * EmuDis_FT_FS_FD( EMU_U32, EMU_U32 Code )
{
    ...
    sprintf( Local_dis_str, " FPR[ %d ], FPR[ %d ], FPR[ %d ]",
            FD,
            FS,
            FT );
...
char * EmuDis_BASE_FT_OFF16( EMU_U32, EMU_U32 Code )
{
    ...
    sprintf( Local_dis_str, " FPR[ %d ], $%x(%s)",
            FT,
            OFF16,
            EmuDis_GetRegName( BASE ) );
...
char * EmuDis_SYNC_STYPE( EMU_U32, EMU_U32 Code )
{
    ...
    if ( SType )
    {
        strcpy( Local_dis_str, ".p" );
    }
    else
    {
        strcpy( Local_dis_str, ".l" );
    }
...
/*    EMU_U32 DEST = ( Code >> 21 ) & 0x0000000F;
    ...
    sprintf( Local_dis_str, "%s %s, VI[%u]",
				( Code & 0x01 ? ".I" : ".NI" ),
...
char * EmuDis_VU_DEST_FT_FS( EMU_U32, EMU_U32 Code )
{
    ...
    EMU_U32 FT = ( Code >> 16 ) & 0x0000001F;
    EMU_U32 FS = ( Code >> 11 ) & 0x0000001F;
    sprintf( Local_dis_str, ".%s%s%s%s vf[%d], vf[%d]",
...
char * EmuDis_BASE_VFT_OFF16( EMU_U32, EMU_U32 Code )
{
    ...
    sprintf( Local_dis_str, " vf[%d], $%x(%s)",
...
char * EmuDis_VU_DEST_FT_FS_FD( EMU_U32, EMU_U32 Code )
{
    ...
    EMU_U32 FS = ( Code >> 11 ) & 0x0000001F;
    EMU_U32 FD = ( Code >>  6 ) & 0x0000001F;
    sprintf( Local_dis_str, ".%s%s%s%s vf[%d], vf[%d], vf[%d]",
       DEST & 0x8 ? "x" : "", DEST & 0x4 ? "y" : "",
       DEST & 0x2 ? "z" : "", DEST & 0x1 ? "w" : "",
       FD, FS, FT );
...
char * EmuDis_VU_DEST_FT_FS_BC( EMU_U32, EMU_U32 Code )
{
    ...
    EMU_U32 FS = ( Code >> 11 ) & 0x0000001F;
    EMU_U32 BC = ( Code ) & 0x00000003;
    sprintf( Local_dis_str, ".%s%s%s%s ACC, vf[%d], vf[%d]",
